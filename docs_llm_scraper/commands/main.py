"""
Main command for crawling and processing documentation sites.

Provides the core functionality for creating docs-llm-pkg packages.
"""
import os
import logging
import tempfile
...
def main(
    ...
    try:
        ...
        # Ensure config path is absolute - convert Path to string
        if isinstance(config, Path):
            ...
            logger.warning(f"Unexpected config type: {type(config)}, defaulting to config.json")
        ...
        outdir_path = os.path.abspath(str(outdir))
